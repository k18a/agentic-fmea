...
class TaxonomyLoader:
    ...
    def search_failure_modes(self, query: str) -> List[FailureMode]:
        ...
        for mode in all_modes.values():
            if (query_lower in mode.description.lower() or 
                query_lower in mode.example.lower() or
                query_lower in mode.potential_impact.lower()):
                results.append(mode)
    ...
    def validate_taxonomy(self) -> List[str]:
        ...
        try:
            ...
            required_categories = ["novel_security", "novel_safety", "existing_security", "existing_safety"]
                ...
                for mode_id, mode_data in modes.items():
                    ...
                    for field in required_fields:
                        if field not in mode_data:
                            errors.append(f"Missing required field '{field}' in {category}.{mode_id}")
                    
                    # Validate pillar values
                    if mode_data.get("pillar") not in ["security", "safety"]:
                        errors.append(f"Invalid pillar value in {category}.{mode_id}: {mode_data.get('pillar')}")
                    
                    # Validate novel field consistency
                    is_novel = "novel" in category
                    if mode_data.get("novel") != is_novel:
                        errors.append(f"Inconsistent novel field in {category}.{mode_id}")
    ...
    def get_taxonomy_stats(self) -> Dict[str, Any]:
        ...
        for category in ["novel_security", "novel_safety", "existing_security", "existing_safety"]:
            stats["by_category"][category] = len(self.get_failure_modes_by_category(category))
