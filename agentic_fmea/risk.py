...
class RiskCalculator:
    ...
    def _get_severity_label(self, severity: int) -> str:
        ...
        labels = {
            ...
            6: "Severe", 7: "Critical", 8: "Very Critical", 9: "Catastrophic", 10: "Extreme"
        }
    ...
    def _get_occurrence_label(self, occurrence: int) -> str:
        ...
        labels = {
            ...
            6: "Moderately High", 7: "High", 8: "Very High", 9: "Extremely High", 10: "Certain"
        }
    ...
    def _get_detection_label(self, detection: int) -> str:
        ...
        labels = {
            1: "Very High (Easily Detected)", 2: "High", 3: "Moderately High", 4: "Moderate", 5: "Low",
            6: "Moderately Low", 7: "Low", 8: "Very Low", 9: "Extremely Low", 10: "Cannot Detect"
        }
    ...
    def analyze_report_risk(self, report: FMEAReport) -> Dict[str, Any]:
        ...
        # Top risk entries
        top_risks = sorted(report.entries, key=lambda x: x.rpn, reverse=True)[:10]
        ...
        for entry in report.entries:
            ...
            if subsystem not in subsystem_risk:
                subsystem_risk[subsystem] = {"count": 0, "total_rpn": 0, "max_rpn": 0}
            
            subsystem_risk[subsystem]["count"] += 1
            subsystem_risk[subsystem]["total_rpn"] += entry.rpn
            subsystem_risk[subsystem]["max_rpn"] = max(subsystem_risk[subsystem]["max_rpn"], entry.rpn)
        
        # Calculate average RPN per subsystem
        for subsystem in subsystem_risk:
            subsystem_risk[subsystem]["avg_rpn"] = (
                subsystem_risk[subsystem]["total_rpn"] / subsystem_risk[subsystem]["count"]
            )
        
        return {
            "statistics": stats,
            "risk_distribution": risk_distribution,
            "top_risks": [{"id": entry.id, "rpn": entry.rpn, "taxonomy_id": entry.taxonomy_id} 
                         for entry in top_risks],
            "subsystem_risk": subsystem_risk
        }
    
    def generate_risk_matrix(self, entries: List[FMEAEntry], 
                           x_axis: str = "occurrence", 
                           y_axis: str = "severity") -> np.ndarray:
        """
    ...
    def plot_risk_matrix(self, entries: List[FMEAEntry], 
                        x_axis: str = "occurrence", 
                        y_axis: str = "severity",
                        title: str = "Risk Matrix") -> plt.Figure:
        """
