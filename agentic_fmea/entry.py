...
class FMEAReport:
    ...
    def high_risk_entries(self) -> List[FMEAEntry]:
        ...
        return [entry for entry in self.entries if entry.risk_level in ["High", "Critical"]]
    ...
    def get_entries_by_subsystem(self, subsystem: Subsystem) -> List[FMEAEntry]:
        ...
        return [entry for entry in self.entries if entry.subsystem == subsystem]
    ...
    def get_entries_by_taxonomy(self, taxonomy_id: str) -> List[FMEAEntry]:
        ...
        return [entry for entry in self.entries if entry.taxonomy_id == taxonomy_id]